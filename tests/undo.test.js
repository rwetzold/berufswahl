...
describe('undo session flow', () => {
  it('stores the current session before applying a valid choice', () => {
    ...
    expect(result.session.ranked.map((song) => song.id)).toEqual(['armenia', 'albania']);
    expect(result.history).toHaveLength(1);
    expect(result.history[0].currentPair.map((song) => song.id)).toEqual(['albania', 'armenia']);
  });

  it('steps back through stored history before using repair fallback', () => {
    const openingSession = createRankingSession(songs);
    const nextSession = choosePreferred(openingSession, 'armenia');

    const result = stepBackSession(nextSession, [openingSession]);

    expect(result.changed).toBe(true);
    expect(result.session.currentPair.map((song) => song.id)).toEqual(['albania', 'armenia']);
